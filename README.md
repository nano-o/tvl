...
How do we check with a SAT solver that the Stellar network is intertwined?

Let's take a federated Byzantine agreement system (FBAS) like the current Stellar network.
...
According to the rules of SCP, two things should hold in the end state for every x in {0,1} and every validator v:
- P1(x,v): If v has a slice that unanimously decides for x, then v must also decide x
- P2(x,v): If v has a blocking set that unanimous decides for x, then v must also decide x
